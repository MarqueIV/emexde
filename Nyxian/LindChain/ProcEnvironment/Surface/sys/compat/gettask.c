...
{    
     ...
     */
    if(!proc_snapshot_permitive_over_pid_allowed(sys_proc_snapshot_, pid, name_only ? PEEntitlementNone : PEEntitlementTaskForPid, name_only ? PEEntitlementNone : PEEntitlementGetTaskAllowed))
    {
        kvo_release(target);
